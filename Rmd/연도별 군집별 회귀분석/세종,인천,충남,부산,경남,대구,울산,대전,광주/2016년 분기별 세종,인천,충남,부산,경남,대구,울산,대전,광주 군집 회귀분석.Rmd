...
##  2016년 분기별 세종,인천,충남,부산,경남,대구,울산,대전,광주
...
analysis_2016_quarter1 <- analysis_2016_quarter1 %>% filter(시도 %in% c("세종","인천","충남","부산","경남","대구","울산","대전","광주")) 

analysis_2016_quarter1 <- analysis_2016_quarter1 %>% dplyr::select(-일시,-시도,-PM10등급,-PM25등급,-년도,-인구수,-`최다풍향(16방위)`)
...
analysis_2016_quarter2 <- analysis_2016_quarter2 %>% filter(시도 %in% c("세종","인천","충남","부산","경남","대구","울산","대전","광주")) 

analysis_2016_quarter2 <- analysis_2016_quarter2 %>% dplyr::select(-일시,-시도,-PM10등급,-PM25등급,-년도,-인구수,-`최다풍향(16방위)`)
...
analysis_2016_quarter3 <- analysis_2016_quarter3 %>% filter(시도 %in% c("세종","인천","충남","부산","경남","대구","울산","대전","광주"))

analysis_2016_quarter3 <- analysis_2016_quarter3 %>% dplyr::select(-일시,-시도,-PM10등급,-PM25등급,-년도,-인구수,-`최다풍향(16방위)`)
...
analysis_2016_quarter4 <- analysis_2016_quarter4 %>% filter(시도 %in% c("세종","인천","충남","부산","경남","대구","울산","대전","광주")) 

analysis_2016_quarter4 <- analysis_2016_quarter4 %>% dplyr::select(-일시,-시도,-PM10등급,-PM25등급,-년도,-인구수,-`최다풍향(16방위)`)
...
```{r}
fit <-  lm(formula = 발병률 ~ CO + PM10 + O3 + `평균 현지기압(hPa)` + 
    `평균기온(°C)` + `평균 풍속(m/s)` + `강수 계속시간(hr)` + 
    `일강수량(mm)` + `일 최심신적설(cm)`, data =  analysis_2016_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ CO + SO2 + PM25 + O3 + `평균 풍속(m/s)` + 
    `강수 계속시간(hr)` + `일강수량(mm)` + `평균기온(°C)` + 
    PM10 + NO2, data = analysis_2016_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ CO + SO2 + PM10 + PM25 + `평균기온(°C)` + 
    `평균 현지기압(hPa)`,  
...
```{r}
fit<-lm(formula = 발병률 ~ CO + PM25 + PM10 + `평균 풍속(m/s)` + O3 + 
    NO2 + `강수 계속시간(hr)` + `평균 현지기압(hPa)` + `평균기온(°C)`,  data = analysis_2016_quarter4)
