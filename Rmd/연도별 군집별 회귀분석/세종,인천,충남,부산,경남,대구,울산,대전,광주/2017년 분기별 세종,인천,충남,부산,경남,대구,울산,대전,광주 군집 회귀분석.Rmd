...
##  2017년 분기별 세종,인천,충남,부산,경남,대구,울산,대전,광주
...
analysis_2017_quarter1 <- analysis_2017_quarter1 %>% filter(시도 %in% c("세종","인천","충남","부산","경남","대구","울산","대전","광주")) 

analysis_2017_quarter1 <- analysis_2017_quarter1 %>% dplyr::select(-일시,-시도,-PM10등급,-PM25등급,-년도,-인구수,-`최다풍향(16방위)`)
...
analysis_2017_quarter2 <- analysis_2017_quarter2 %>% filter(시도 %in% c("세종","인천","충남","부산","경남","대구","울산","대전","광주")) 

analysis_2017_quarter2 <- analysis_2017_quarter2 %>% dplyr::select(-일시,-시도,-PM10등급,-PM25등급,-년도,-인구수,-`최다풍향(16방위)`)
...
analysis_2017_quarter3 <- analysis_2017_quarter3 %>% filter(시도 %in% c("세종","인천","충남","부산","경남","대구","울산","대전","광주"))

analysis_2017_quarter3 <- analysis_2017_quarter3 %>% dplyr::select(-일시,-시도,-PM10등급,-PM25등급,-년도,-인구수,-`최다풍향(16방위)`)
...
analysis_2017_quarter4 <- analysis_2017_quarter4 %>% filter(시도 %in% c("세종","인천","충남","부산","경남","대구","울산","대전","광주")) 

analysis_2017_quarter4 <- analysis_2017_quarter4 %>% dplyr::select(-일시,-시도,-PM10등급,-PM25등급,-년도,-인구수,-`최다풍향(16방위)`)
...
```{r}
fit <-  lm(formula = 발병률 ~ `평균 풍속(m/s)` + `평균기온(°C)` + SO2 + 
    `강수 계속시간(hr)` + NO2 + CO + PM25, data =  analysis_2017_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ `평균 풍속(m/s)` + CO + NO2 + PM25 + SO2 + 
    PM10 + `강수 계속시간(hr)` + `일강수량(mm)` + `평균기온(°C)`,  data = analysis_2017_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ CO + SO2 + PM25 + PM10 + `평균기온(°C)` + 
    `강수 계속시간(hr)` + `평균 풍속(m/s)` + `평균 현지기압(hPa)` + 
    NO2,   
...
```{r}
fit<-lm(formula = 발병률 ~ SO2 + CO + PM25 + PM10 + `평균기온(°C)` + 
    NO2 + O3 + `일 최심신적설(cm)` + `평균 풍속(m/s)`,  data = analysis_2017_quarter4)
