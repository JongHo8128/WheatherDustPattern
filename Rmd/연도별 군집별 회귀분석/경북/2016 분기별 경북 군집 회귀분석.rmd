...
fit <-  lm(formula = 발병률 ~ NO2 + PM25 + PM10 + `평균기온(°C)` + `일강수량(mm)` + 
    `평균 풍속(m/s)` + `평균 현지기압(hPa)` + O3, data =  analysis_2016_quarter1)
...
fit <- lm(formula = 발병률 ~ NO2 + PM25 + PM10 + `평균 풍속(m/s)` + 
    ...
    SO2 + CO + O3, data = analysis_2016_quarter2)
...
fit<- lm(formula = 발병률 ~ NO2 + PM25 + O3 + `평균 현지기압(hPa)` + 
    `평균기온(°C)` + CO + `평균 풍속(m/s)` + SO2 + `강수 계속시간(hr)`,  data = analysis_2016_quarter4)
