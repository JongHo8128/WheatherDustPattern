...
analysis_2018_quarter1 <- analysis_2018_quarter1 %>% filter(시도 %in% c("경북")) 
...
analysis_2018_quarter2 <- analysis_2018_quarter2 %>% filter(시도 %in% c("경북")) 
...
analysis_2018_quarter3 <- analysis_2018_quarter3 %>% filter(시도 %in% c("경북"))
...
analysis_2018_quarter4 <- analysis_2018_quarter4 %>% filter(시도 %in% c("경북")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ NO2 + PM25 + PM10 + `평균기온(°C)` + `일강수량(mm)` + 
    `평균 풍속(m/s)` + `평균 현지기압(hPa)` + O3, data =  analysis_2018_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ NO2 + PM25 + PM10 + `평균 풍속(m/s)` + 
    `평균 현지기압(hPa)` + `평균기온(°C)` + `강수 계속시간(hr)` + 
    SO2 + CO + O3, data = analysis_2018_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + PM25 + `평균 현지기압(hPa)` + CO + 
    O3 + PM10,  
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + PM25 + O3 + `평균 현지기압(hPa)` + 
    `평균기온(°C)` + CO + `평균 풍속(m/s)` + SO2 + `강수 계속시간(hr)`,  data = analysis_2018_quarter4)
