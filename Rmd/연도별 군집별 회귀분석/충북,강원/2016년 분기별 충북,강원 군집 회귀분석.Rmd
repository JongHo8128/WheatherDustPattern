...
analysis_2016_quarter1 <- analysis_2016_quarter1 %>% filter(시도 %in% c("충북","강원")) 
...
analysis_2016_quarter2 <- analysis_2016_quarter2 %>% filter(시도 %in% c("충북","강원")) 
...
analysis_2016_quarter3 <- analysis_2016_quarter3 %>% filter(시도 %in% c("충북","강원"))
...
analysis_2016_quarter4 <- analysis_2016_quarter4 %>% filter(시도 %in% c("충북","강원")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ NO2 + PM25 + O3 + SO2 + `평균 풍속(m/s)` + 
    `평균 현지기압(hPa)` + `일강수량(mm)`, data =  analysis_2016_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ NO2 + SO2 + `평균 풍속(m/s)` + CO, data = analysis_2016_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + SO2 + `평균 현지기압(hPa)` + `평균기온(°C)`,  
...
```{r}
fit<-lm(formula = 발병률 ~ NO2 + `평균 현지기압(hPa)` + CO + `평균기온(°C)`,  data = analysis_2016_quarter4)
