...
analysis_2018_quarter1 <- analysis_2018_quarter1 %>% filter(시도 %in% c("충북","강원")) 
...
analysis_2018_quarter2 <- analysis_2018_quarter2 %>% filter(시도 %in% c("충북","강원")) 
...
analysis_2018_quarter3 <- analysis_2018_quarter3 %>% filter(시도 %in% c("충북","강원"))
...
analysis_2018_quarter4 <- analysis_2018_quarter4 %>% filter(시도 %in% c("충북","강원")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ `평균기온(°C)` + NO2 + CO + `평균 현지기압(hPa)`, data =  analysis_2018_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ 1, data = analysis_2018_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + SO2 + `평균 풍속(m/s)` + `평균 현지기압(hPa)` + 
    `평균기온(°C)` + O3 + `일강수량(mm)`,  
...
```{r}
fit<-lm(formula = 발병률 ~ NO2 + SO2 + `평균 풍속(m/s)` + PM25,  data = analysis_2018_quarter4)
