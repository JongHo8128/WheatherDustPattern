...
analysis_2017_quarter1 <- analysis_2017_quarter1 %>% filter(시도 %in% c("충북","강원")) 
...
analysis_2017_quarter2 <- analysis_2017_quarter2 %>% filter(시도 %in% c("충북","강원")) 
...
analysis_2017_quarter3 <- analysis_2017_quarter3 %>% filter(시도 %in% c("충북","강원"))
...
analysis_2017_quarter4 <- analysis_2017_quarter4 %>% filter(시도 %in% c("충북","강원")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ `평균 풍속(m/s)` + NO2 + `강수 계속시간(hr)` + 
    `일강수량(mm)` + SO2, data =  analysis_2017_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ SO2 + NO2 + `평균기온(°C)` + CO + `평균 현지기압(hPa)` + 
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + SO2 + `평균 풍속(m/s)` + `일강수량(mm)`,   
...
```{r}
fit<-lm(formula = 발병률 ~ NO2 + `평균 현지기압(hPa)` + CO + O3,  data = analysis_2017_quarter4)
