...
analysis_2017_quarter1 <- analysis_2017_quarter1 %>% filter(시도 %in% c("전남","제주")) 
...
analysis_2017_quarter2 <- analysis_2017_quarter2 %>% filter(시도 %in% c("전남","제주")) 
...
analysis_2017_quarter3 <- analysis_2017_quarter3 %>% filter(시도 %in% c("전남","제주"))
...
analysis_2017_quarter4 <- analysis_2017_quarter4 %>% filter(시도 %in% c("전남","제주")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ SO2 + NO2 + `평균 풍속(m/s)` + `평균 현지기압(hPa)` + 
    `평균기온(°C)` + CO + PM25, data = analysis_2017_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ SO2 + NO2 + CO + `평균 풍속(m/s)` + 
    `평균기온(°C)` + `평균 현지기압(hPa)` + PM25 + 
    PM10, data = analysis_2017_quarter2)
...
```{r}
fitdata <- analysis_2017_quarter4 %>% dplyr::select(-`일 최심신적설(cm)`)
...
```{r}
fit<- lm(formula = 발병률 ~ SO2 + NO2 + O3 + CO + `평균기온(°C)`, data =analysis_2017_quarter4)
