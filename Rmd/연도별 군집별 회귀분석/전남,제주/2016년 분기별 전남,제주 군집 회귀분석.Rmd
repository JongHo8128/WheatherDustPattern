...
analysis_2016_quarter1 <- analysis_2016_quarter1 %>% filter(시도 %in% c("전남","제주")) 
...
analysis_2016_quarter2 <- analysis_2016_quarter2 %>% filter(시도 %in% c("전남","제주")) 
...
analysis_2016_quarter3 <- analysis_2016_quarter3 %>% filter(시도 %in% c("전남","제주"))
...
analysis_2016_quarter4 <- analysis_2016_quarter4 %>% filter(시도 %in% c("전남","제주")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ SO2 + `평균기온(°C)` + `평균 현지기압(hPa)` + 
    `평균 풍속(m/s)` + CO + NO2 + O3 + PM25 + PM10, data = analysis_2016_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ SO2 + `평균기온(°C)` + `평균 현지기압(hPa)` + 
    NO2 + CO + PM25 + `평균 풍속(m/s)` + PM10, data = analysis_2016_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ SO2 + `평균 현지기압(hPa)` + PM10 + 
    PM25 + NO2, 
...
```{r}
fit<- lm(formula = 발병률 ~ SO2 + NO2 + PM10 + CO, data =analysis_2016_quarter4)
