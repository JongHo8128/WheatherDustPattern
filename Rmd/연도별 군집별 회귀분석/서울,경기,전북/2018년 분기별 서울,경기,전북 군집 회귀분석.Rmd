...
analysis_2018_quarter1 <- analysis_2018_quarter1 %>% filter(시도 %in% c("서울","경기","전북")) 
...
analysis_2018_quarter2 <- analysis_2018_quarter2 %>% filter(시도 %in% c("서울","경기","전북")) 
...
analysis_2018_quarter3 <- analysis_2018_quarter3 %>% filter(시도 %in% c("서울","경기","전북"))
...
analysis_2018_quarter4 <- analysis_2018_quarter4 %>% filter(시도 %in% c("서울","경기","전북")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ NO2 + CO + PM25 + `평균기온(°C)` + `평균 풍속(m/s)` + 
    `평균 현지기압(hPa)` + O3 + PM10 + `일강수량(mm)`, data =  analysis_2018_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ NO2 + CO + SO2 + PM25 + `평균 풍속(m/s)` + 
    `평균 현지기압(hPa)` + `강수 계속시간(hr)`, data = analysis_2018_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + CO + `평균 현지기압(hPa)` + SO2 + 
    `강수 계속시간(hr)` + `평균기온(°C)`,  
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + PM25 + SO2 + O3 + `평균 현지기압(hPa)` + 
    `평균 풍속(m/s)` + `평균기온(°C)` + `일강수량(mm)` + `강수 계속시간(hr)` + 
    CO + PM10,  data = analysis_2018_quarter4)
