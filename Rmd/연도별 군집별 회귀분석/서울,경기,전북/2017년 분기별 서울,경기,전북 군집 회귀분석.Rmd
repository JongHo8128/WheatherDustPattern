...
analysis_2017_quarter1 <- analysis_2017_quarter1 %>% filter(시도 %in% c("서울","경기","전북")) 

analysis_2017_quarter1 <- analysis_2017_quarter1 %>% mutate(발생율 = 발생건수/인구수*10000)

analysis_2017_quarter1 <- analysis_2017_quarter1 %>% select(-일시,-시도,-PM10등급,-PM25등급,-년도,-인구수,-`최다풍향(16방위)`)
...
analysis_2017_quarter2 <- analysis_2017_quarter2 %>% filter(시도 %in% c("서울","경기","전북")) 

analysis_2017_quarter2 <- analysis_2017_quarter2 %>% mutate(발생율 = 발생건수/인구수*10000)

analysis_2017_quarter2 <- analysis_2017_quarter2 %>% select(-일시,-시도,-PM10등급,-PM25등급,-년도,-인구수,-`최다풍향(16방위)`)
...
analysis_2017_quarter3 <- analysis_2017_quarter3 %>% filter(시도 %in% c("서울","경기","전북"))

analysis_2017_quarter3 <- analysis_2017_quarter3 %>% mutate(발생율 = 발생건수/인구수*10000)

analysis_2017_quarter3 <- analysis_2017_quarter3 %>% select(-일시,-시도,-PM10등급,-PM25등급,-년도,-인구수,-`최다풍향(16방위)`,-`일 최심신적설(cm)`,-`일강수량(mm)`,-`강수 계속시간(hr)`)
...
analysis_2017_quarter4 <- analysis_2017_quarter4 %>% filter(시도 %in% c("서울","경기","전북")) 

analysis_2017_quarter4 <- analysis_2017_quarter4 %>% mutate(발생율 = 발생건수/인구수*10000)

analysis_2017_quarter4 <- analysis_2017_quarter4 %>% select(-일시,-시도,-PM10등급,-PM25등급,-년도,-인구수,-`최다풍향(16방위)`)
...
fit <-  lm(formula = 발생건수 ~ 발생율 + `평균 풍속(m/s)` + SO2 + `강수 계속시간(hr)` + 
    ...
    PM10, data = analysis_2017_quarter1)
...
fit <- lm(formula = 발생건수 ~ 발생율 + `평균 풍속(m/s)` + CO + NO2 + 
    ...
    `일강수량(mm)` + O3 + SO2, data = analysis_2017_quarter2)
...
fit<- lm(formula = 발생건수 ~ 발생율 + `평균 풍속(m/s)` + CO + PM25 + 
    ...
    `평균기온(°C)` + `일 최심신적설(cm)`, data =analysis_2017_quarter4)
...
fit <-  lm(formula = 발생율 ~ 발생건수 + `평균 현지기압(hPa)` + `강수 계속시간(hr)` + 
    PM25 + NO2 + SO2 + PM10 + `일강수량(mm)` + `평균기온(°C)`, data = analysis_2017_quarter1)
...
fit <- lm(formula = 발생율 ~ 발생건수 + CO + SO2 + PM10 + `평균 풍속(m/s)` + 
    `일강수량(mm)` + `강수 계속시간(hr)` + PM25 + NO2, data = analysis_2017_quarter2)
...
fit<- lm(formula = 발생율 ~ 발생건수 + NO2 + PM25 + PM10 + CO + SO2 + 
    O3 + `평균 현지기압(hPa)`, data =analysis_2017_quarter4)
