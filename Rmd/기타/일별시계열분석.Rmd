...
analysis_sido_day <- analysis_total %>% group_by(일시) %>% summarise(발병율 = sum(발생건수,na.rm=TRUE)/sum(인구수,na.rm=TRUE))

ts <- ts(analysis_sido_day[-1]$발병율,frequency = 365, start = c(2016,1))
