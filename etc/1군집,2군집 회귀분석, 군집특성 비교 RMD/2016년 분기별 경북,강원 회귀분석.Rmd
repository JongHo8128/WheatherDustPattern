...
analysis_2016_quarter1 <- analysis_2016_quarter1 %>% filter(시도 %in% c("경북","강원")) 
...
analysis_2016_quarter2 <- analysis_2016_quarter2 %>% filter(시도 %in% c("경북","강원")) 
...
analysis_2016_quarter3 <- analysis_2016_quarter3 %>% filter(시도 %in% c("경북","강원"))
...
analysis_2016_quarter4 <- analysis_2016_quarter4 %>% filter(시도 %in% c("경북","강원")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ PM25 + NO2 + PM10 + SO2 + O3, data = analysis_2016_quarter1)

summary(fit)

# 1군집(경북, 강원)
# 회귀분석 결과 PM25, NO2 0.001에서 유의,
#               PM10, SO2 0.05에서 유의
#
# 군집별 분기별 비교 결과 발병률(2) - (
#                                       PM25(4)불일치, NO2(2)일치,
...
```{r}
fit <- lm(formula = 발병률 ~ NO2 + PM25 + `평균 풍속(m/s)` + CO, data = analysis_2016_quarter2)

summary(fit)

# 1군집(경북, 강원)
# 회귀분석 결과 NO2 0.001에서 유의,
#               평균풍속 0.1에서 유의
#
# 군집별 분기별 비교 결과 발병률(1) - (
#                                       NO2(2)일치,
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + `평균기온(°C)` + PM25 + SO2 + `평균 풍속(m/s)`, 
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + CO + PM25 + PM10, data =analysis_2016_quarter4)
